...
{
    bool put_brackets_right = do_I_put_brackets(tree->right)||(!strcmp(operat,"\\cdot")),
         put_brackets_left  = do_I_put_brackets(tree->left)||(!strcmp(operat, "\\cdot"));
...
void Print_POW(FILE* TEX, Tree* tree)
{
    fprintf    (TEX , "(");
    Print_Tree (TEX , tree->left);
    fprintf    (TEX , ")");
...
void Print_OPERATOR(FILE* TEX, Tree* tree)
{
    ...
    else if(!strcmp("^", tree->value))   Print_POW(TEX, tree);

    else if(!strcmp("cos", tree->value)) Print_Unar(TEX, tree->right, "cos");
    else if(!strcmp("sin", tree->value)) Print_Unar(TEX, tree->right, "sin");
    else if(!strcmp("tg", tree->value))  Print_Unar(TEX, tree->right, "tg");
    else if(!strcmp("ctg", tree->value)) Print_Unar(TEX, tree->right, "ctg");

    else if(!strcmp("arccos", tree->value)) Print_Unar(TEX, tree->right, "arccos");
    else if(!strcmp("arcsin", tree->value)) Print_Unar(TEX, tree->right, "arcsin");
    else if(!strcmp("arctg", tree->value))  Print_Unar(TEX, tree->right, "arctg");
    else if(!strcmp("arcctg", tree->value)) Print_Unar(TEX, tree->right, "arcctg");

    else if(!strcmp("ln", tree->value))   Print_Unar(TEX, tree->right, "ln");
    else if(!strcmp("exp", tree->value))  Print_Unar(TEX, tree->right, "exp");
}
